...
skill: research
engine: claude
summary: wisper-core so far is pure logic — settings, dictionary, formatting, model catalog, prompts and an LLM cleanup client — with no audio code at all yet. Phase 2 needs a new crate wired with cpal, Silero VAD and rubato, plus a workspace-member and Cargo.toml update, since RESEARCH.md actually files audio capture under Phase 1, not Phase 2.
tags: wisper-core, research, phase2, audio, vad
---
...
4. VAD auto-stop threshold/behavior (RESEARCH.md cites local-whisper's "stop after 3 s below −40 dB" as a reference default) and a buffer format that feeds directly into the (also unimplemented) whisper.cpp ASR stage.
5. Tests: the existing 7 modules are all unit-tested (pure logic, no hardware). Audio capture is hardware-dependent, so the new code will need a mockable/injectable capture abstraction to keep `python3 -m unittest`/`cargo test`-style coverage meaningful, per this repo's "ship-check" gate expectations.

Note: I was not able to run `cargo test`/`cargo check` in this session (the command required approval that wasn't granted), so current green/red build status is unverified here — `repo-pulse` last reported `cargo check: ✅ clean` on this branch.
...
## Open questions
- Should the phase numbering be reconciled (directive says "Phase 2" for audio/VAD, RESEARCH.md's own plan calls it Phase 1, the last commit labeled unrelated cleanup/dictionary/settings work "Phase 1")? Recommend renaming/relabeling before more work lands so directives and RESEARCH.md agree.
